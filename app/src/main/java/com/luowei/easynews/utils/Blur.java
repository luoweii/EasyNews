...
public final class Blur {
     ...
     * @param radius 模糊取样值
     * @param scaleFactor 缩小值(0.0-1.0)
     * @return
     */
    public static Bitmap fastblur(Bitmap sentBitmap, int radius,float scaleFactor) {
        if (scaleFactor > 1) {
            scaleFactor = 1;
        }

        radius = (int)((scaleFactor*10)*radius/10);
        if (radius==0) {
            radius = 1;
        }
        // 取得想要缩放的matrix参数
        ...
        sentBitmap = doBlur(sentBitmap, radius, true);
        return sentBitmap;
    }
